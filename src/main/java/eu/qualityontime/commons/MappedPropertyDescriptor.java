...
/**
 * A MappedPropertyDescriptor describes one mapped property. Mapped properties
 * are multivalued properties like indexed properties but that are accessed with
 * a String key instead of an index. Such property values are typically stored
 * in a Map collection. For this class to work properly, a mapped value must
 * have getter and setter methods of the form
 * <p>
 * <code>get<strong>Property</strong>(String key)</code> and
 * <p>
 * <code>set<strong>Property</strong>(String key, Object value)</code>,
 * <p>
 * where <code><strong>Property</strong></code> must be replaced by the name of
 * the property.
 * 
...
public class MappedPropertyDescriptor extends PropertyDescriptor {
	...
	/**
	 * Constructs a MappedPropertyDescriptor for a property that follows the
	 * standard Java convention by having getFoo and setFoo accessor methods,
	 * with the addition of a String parameter (the key). Thus if the argument
	 * name is "fred", it will assume that the writer method is "setFred" and
	 * the reader method is "getFred". Note that the property name should start
	 * with a lower case character, which will be capitalized in the method
	 * names.
	 *
	 * @param propertyName
	...
	public MappedPropertyDescriptor(final String propertyName, final Class<?> beanClass) throws IntrospectionException {
		...
		if (propertyName == null || propertyName.length() == 0) {
			throw new IntrospectionException(
					"bad property name: " + propertyName + " on class: " + beanClass.getClass().getName());
		}
	 ...
	 *            The name of the method used for reading one of the property
	 *            values. May be null if the property is write-only.
	 * @param mappedSetterName
	 *            The name of the method used for writing one of the property
	 *            values. May be null if the property is read-only.
	 *
	 * @throws IntrospectionException
	 *             if an exception occurs during introspection.
	 */
	...
	/**
	 * This constructor takes the name of a mapped property, and Method objects
	 * for reading and writing the property.
	 *
	 * @param propertyName
	 *            The programmatic name of the property.
	 * @param mappedGetter
	 *            The method used for reading one of the property values. May be
	 *            be null if the property is write-only.
	 * @param mappedSetter
	 *            The method used for writing one the property values. May be
	 ...
	 *
	 * @return The Java type info for the property values. Note that the "Class"
	 *         object may describe a built-in Java type such as "int". The
	 *         result may be "null" if this is a mapped property that does not
	 *         support non-keyed access.
	 *         <p>
	...
	/**
	 * Introspect our bean class to identify the corresponding getter and setter
	 * methods.
	 */
		...
		try {
			...
			if (mappedReadMethod != null) {
				...
				if (mappedPropertyType == Void.TYPE) {
					throw new IntrospectionException(
							"mapped read method " + mappedReadMethod.getName() + " returns void");
				}
			}
	...
	private static Method getMethod(final Class<?> clazz, final String methodName, final Class<?>[] parameterTypes)
		...
		// No Method found
		throw new IntrospectionException(
				"No method \"" + methodName + "\" with " + parameterCount + " parameter(s) of matching types.");
	}

	/**
	 * Holds a {@link Method} in a {@link SoftReference} so that it it doesn't
	 * prevent any ClassLoader being garbage collected, but tries to re-create
	 * the method if the method reference has been released.
	 *
